...
def compute_gradient(img):
    # grad_x = F.conv2d(img, torch.Tensor([[-1, 0, 1]]).view(1, 1, 1, 3), padding=(0, 1))
    # grad_y = F.conv2d(img, torch.Tensor([[-1], [0], [1]]).view(1, 1, 3, 1), padding=(1, 0))
    grad_x = img[:, :, :, 1:] - img[:, :, :, :-1]
    grad_y = img[:, :, 1:, :] - img[:, :, :-1, :]
    return grad_x, grad_y

def conf_loss(flow, flow_gt, conf, valid):
    error = torch.norm((flow - flow_gt), p=2, dim=1, keepdim=True)
    # print(conf.shape)
    gt_conf = torch.sigmoid(-(error - 1) * 5)
    conf_loss = torch.abs((gt_conf - conf) * valid.unsqueeze(1))
...
def train_dist(args, world_size):
    ...
    # pre_train = torch.load('pretrain/DIP_stereo.pth')
    ...
    for epoch_idx in range(start_epoch_idx, args.n_total_epoch + 1):
        ...
        for batch_idx, mini_batch_data in enumerate(dataloader):
            ...
            left, right, gt_disp = (
                ...
                mini_batch_data["disp"].to(local_rank),
            )
...
def train(args, world_size):
    ...
    if chk_path is not None:
        ...
        model.module.load_state_dict(state_dict['state_dict'], strict=False)
        # optimizer.load_state_dict(state_dict['optim_state_dict'])
        resume_epoch_idx = state_dict["epoch"]
        resume_iters = state_dict["iters"]
        start_epoch_idx = resume_epoch_idx + 1
