...
class ToTensor(object):
    ...
    def __init__(self, no_normalize=True):
...
class RandomScale(object):
    ...
    def __call__(self, sample):
        if np.random.rand() < 0.5:
            ...
            base_scale = self.crop_width / w
            max_scale = base_scale * 1.25
            min_scale = base_scale * 0.85
            scale = np.random.uniform(min_scale, max_scale)

            # only random scale x axis
            sample['left'] = cv2.resize(sample['left'], None, fx=scale, fy=scale, interpolation=cv2.INTER_LINEAR)
            sample['right'] = cv2.resize(sample['right'], None, fx=scale, fy=scale, interpolation=cv2.INTER_LINEAR)

            sample['disp'] = cv2.resize(
                sample['disp'], None, fx=scale, fy=scale,
            ...
            if 'pseudo_disp' in sample and sample['pseudo_disp'] is not None:
                sample['pseudo_disp'] = cv2.resize(sample['pseudo_disp'], None, fx=scale, fy=scale,
